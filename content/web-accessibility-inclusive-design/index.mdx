...
맞죠... 현재도 부족하지만 궁금하고 해보고싶은 욕구 때문에 제가 직접 만들었을때 가장 큰 장점은 제 마음대로 디자인이 가능한게 가장 큰 매력이였습니다.
블로그 플랫폼에 가장 알려진 네이버 , 티스토리, velog 등등 .. 보다보면 레이아웃만 보면 특정 블로그 플랫폼이 떠오르는 디자인이라고 생각했습니다.
뭐 정말 블로그를 시간내서 잘 꾸미신분들을 보면 엄청 기존의 남들이 쓰는 틀이 아닌 커스텀이 굉장히 잘 만져진곳도 많더라구요.
나는 내 블로그의 나만의 색을 뭔가 좀 표현하고 싶다는 생각에 이러한 디자인을 만들었으며 , 기존 Next.Js 12 Pages Router가 아닌 Next.Js 13 App Router 로 공부하고자 만들었습니다.

> [프로젝트 코드 보러가기 (GitHub Repository)](https://github.com/s2s2hyun/dh-project)

## 2. contentlayer를 사용한 정적 블로그 제작👩‍🌾

contentlayer를 사용하기 위해 먼저 Next.Js 프로젝트를 생성해줍니다.
...
<bold>contentlayer.config.ts</bold>

![Alt text for image](../../public/blogs/nextjsfirst.png 'Optional title')
