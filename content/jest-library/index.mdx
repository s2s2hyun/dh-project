...
이전까지는 프로젝트를 하면서 테스트 코드라는 것을 들어만 봤지 실제로 만져보고 테스트를 해본건 처음이라, Jest 공식문서를 확인해보면 알겠지만 정말 내가 위에서 다룬건 기초적인 것만 만져봐서<br /> 다음엔 React 프로젝트 내에 Jest 라이브러리를 적용해서 코드를 올바르게 썼는지에 대한 테스트를 한번 도전해보겠습니다.
